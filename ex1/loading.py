...
def print_dependency_report(modules: dict[str, Any], api_mode: bool) -> None:
    ...
    if api_mode:
        ...
        status = "OK" if module is not None else "MISSING"
        message = "Network access ready" if module is not None else "Optional API access unavailable"
        print(f"[{status}] requests ({version}) - {message}")


def print_manager_comparison() -> None:
    poetry_active = bool(os.environ.get("POETRY_ACTIVE")) or "poetry" in sys.executable.lower()
    pip_env = sys.prefix != sys.base_prefix

    print("Package manager comparison:")
    print("- pip: installs directly from requirements.txt with `pip install -r requirements.txt`.")
    print("- Poetry: reads pyproject.toml, resolves dependencies, and uses a lockfile with `poetry install`.")
    if poetry_active:
        print("- Current environment: Poetry-managed execution detected.")
    elif pip_env:
        print("- Current environment: virtual environment detected; this is the usual pip workflow.")
    else:
        print("- Current environment: global interpreter detected; package installs would affect the system Python.")
...
def build_matrix_data(np: Any, pd: Any, rows: int = 1000) -> Any:
    ...
    stability = 100 - (np.abs(signal) * 11 + np.abs(rng.normal(0.0, 3.0, size=rows)))
    ...
    frame["rolling_stability"] = frame["stability"].rolling(window=40, min_periods=1).mean()
    frame["zone"] = pd.cut(
        frame["stability"],
        bins=[-0.1, 40, 70, 100],
...
def summarize_matrix_data(frame: Any, api_mode: bool) -> None:
    ...
    zone_counts = frame["zone"].value_counts(dropna=False).reindex(["critical", "unstable", "stable"], fill_value=0)
    ...
    print(f"- matplotlib: {package_version('matplotlib', sys.modules.get('matplotlib'))}")
    if api_mode and sys.modules.get("requests") is not None:
        print(f"- requests: {package_version('requests', sys.modules.get('requests'))}")
...
def create_visualization(frame: Any, plt: Any) -> None:
    ...
    ax1.plot(frame["cycle"], frame["stability"], color="#2ec4b6", alpha=0.35, linewidth=1, label="stability")
    ax1.plot(frame["cycle"], frame["rolling_stability"], color="#e71d36", linewidth=2.2, label="rolling mean")
    ...
    zone_counts = frame["zone"].value_counts().reindex(["critical", "unstable", "stable"], fill_value=0)
    ax2.bar(zone_counts.index.astype(str), zone_counts.values, color=["#ff6b6b", "#f4a261", "#2a9d8f"])
    ax2.set_xlabel("Zone")
...
def show_missing_help(missing_required: list[str], missing_optional: list[str], api_mode: bool) -> int:
